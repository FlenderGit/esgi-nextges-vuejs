...
<template>
  <WidgetBase title="Mes cours" :cols="2" :rows="2">
    <template v-slot:actions>
      <Button @click="navigateToPlanning">Voir plus</Button>
    </template>
      ...
      <div v-for="(classe, i) in classes">
        <div class="flex justify-between items-center px-2 py-1 rounded-xl hover:bg-neutral-200">
          <div class="grid">
            ...
            <p class="text-neutral-600/80 text-xs">
              Salle {{ classe.location }} • Bloc {{ classe.bloc }}
            </p>
          </div>
          <div>
            <p class="text-neutral-600/80 text-xs">
              {{ timestampToTime(classe.start) }} - {{ timestampToTime(classe.end) }}
            </p>
          </div>
        </div>
      </div>
